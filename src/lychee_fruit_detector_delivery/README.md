...
本目录是荔枝果实YOLO目标检测子模块，默认类别为 `lychee`、模型为 `models/best1.pt`、输入尺寸为1696。

## 文件作用

```text
├─ models/best1.pt       # 模型权重
...
```bash
python infer_single.py --image /path/to/lychee.jpg --output-dir ./output/single
...
```bash
python run.py fruit folder --input-dir /path/to/images
python run.py fruit evaluate --input-dir /path/to/dataset --device cpu
```
