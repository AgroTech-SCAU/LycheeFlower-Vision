...
def parse_args() -> argparse.Namespace:
    """解析权重、数据集、评估划分、输出目录和验收阈值。"""
    package_dir = Path(__file__).resolve().parent
    parser = argparse.ArgumentParser(description="荔枝果实 YOLO 测试集评估")
    parser.add_argument(
        "--weights",
    ...
    parser.add_argument("--imgsz", type=int, default=1696, help="评估输入尺寸")
...
def validate_args(args: argparse.Namespace) -> tuple[Path, Path, Path | None]:
    ...
    return weights_path, input_dir, data_path
...
def create_dataset_config(
    ...
    """自动识别带标签验证集目录并生成Ultralytics可读取的配置文件。"""
    candidates = [
        (input_dir / split / "images", input_dir / split / "labels"),
        (input_dir / "images", input_dir / "labels"),
    ]
    selected = next(
        ((images, labels) for images, labels in candidates if images.is_dir() and labels.is_dir()),
        None,
    )
    if selected is None:
...
def resolve_dataset_config(data_path: Path, output_dir: Path) -> tuple[Path, dict[str, Any]]:
    """将不可用的数据集根路径改为 data.yaml 所在目录并保存可复现配置。"""
...
def count_split_images(config: dict[str, Any], split: str) -> int | None:
    """统计配置中指定数据划分的受支持图片数量。"""
    ...
    for value in values:
        ...
        if candidate.is_dir():
            ...
            count += sum(bool(line.strip()) for line in candidate.read_text(encoding="utf-8").splitlines())
    return count


def to_float_list(values: Any) -> list[float]:
    """将 NumPy 或张量形式的指标序列转换为普通浮点数列表。"""
    if values is None:
        return []
...
def build_metrics_payload(
    ...
    """将 Ultralytics 验证指标整理为稳定的验收报告结构。"""
    ...
    return {
        ...
        "dataset": {
            ...
            "split": args.split,
            "image_count": count_split_images(config, args.split),
        ...
        "iou_definition": {
            "formula": "IoU = area(prediction ∩ ground_truth) / area(prediction ∪ ground_truth)",
            "description": "预测类别正确且预测框与真实框的 IoU 不低于0.5时，参与mAP50的正确匹配判定。",
        },
        "metrics": {
            "precision": precision,
            "recall": recall,
...
def write_metrics_files(payload: dict[str, Any], output_dir: Path) -> None:
    """将评估结果写成 JSON、CSV、文本和独立 mAP50 文件。"""
...
def run_evaluation(args: argparse.Namespace) -> dict[str, Any]:
    """运行完整测试集评估并生成验收报告和可视化图表。"""
    weights_path, input_dir, data_path = validate_args(args)
    output_dir = Path(args.output_dir).expanduser().resolve()
